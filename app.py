...
def main():
    st.header("_Tag The_ :orange[Deal] :ok_hand:")
    st.sidebar.header("_Kolayca Bul_ , :orange[Kıyasla] ve :blue[Karşılaştır]!")
    ...
    st.sidebar.image("TTD_logo.png",caption="© Made by Baran Çakı , MIT License")
        ...
        if e_comm_lower in ["hepsiburada"]:
            ...
            if st.button("Getir !"):
                if not product or not range_1 or not range_2 or range_1 > range_2:
                    ...
                    if "pd_table" in st.session_state:
                        st.markdown(':orange[Ürün Markası] ve :blue[Fiyat] Tablosu :')
                        st.area_chart(st.session_state["pd_table"][["Ürün Markası", "Fiyat (Sayı)"]].set_index("Ürün Markası"))
